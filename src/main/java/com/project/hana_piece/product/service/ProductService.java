...
public class ProductService {
    ...
    public RecommendationResponse recommendProducts(Long userId, Long userGoalId) {
        UserGoal userGoal = userGoalRepository.findById(userGoalId).orElseThrow(()-> new UserGoalNotFoundException(userGoalId));

        List<Product> products = productRepository.findAll();
        List<EnrolledProduct> enrolledProducts = enrolledProductRepository.findByUserGoalUserUserId(userId);
        ...
        System.out.println(aiResponseMessage);
    ...
    private GeminiPrompt buildPromptMessage(UserGoal userGoal, List<Product> products, List<EnrolledProduct> enrolledProducts) {
        ...
        constraints.append("아래의 제약 조건들은 중요도를 기준으로 정리한 것이야\n")
        ...
        String responseFormat = "Constraints 우선 순위 기준으로 정렬해서 추천 상품의 product_id만 콤마를 기준으로 공백없이 응답해줘.";
